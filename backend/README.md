...
STORE appearance presets are scoped by the current store tenant and only affect presentation. Admins can read/update them with `GET /api/store/appearance` and `PUT /api/store/appearance` using `{ "preset": "MODERN", "palette": "CORAL", "shape": "ROUNDED" }`. Supported presets are `MODERN`, `CLASSIC`, `LOCAL_BUSINESS` and `PORTFOLIO`; palettes are `CORAL`, `OCEAN`, `FOREST` and `GRAPHITE`; shapes are `SQUARE`, `ROUNDED` and `SOFT`. New stores default to `MODERN`, `CORAL` and `ROUNDED`. Phase 1 uses two simple columns on `stores`; no JSONB, table, or entity is introduced.
