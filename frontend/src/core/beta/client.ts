...
import { Sentry, sentryEnabled } from '@/app/observability/sentry'
...
function buildBody(payload: BetaTelemetryPayload) {
  return JSON.stringify({
    ...
    productSlug: sanitizeText(payload.productSlug, 120),
...
function addTelemetryBreadcrumb(payload: BetaTelemetryPayload) {
  if (!sentryEnabled()) return

  Sentry.addBreadcrumb({
    category: 'beta.telemetry',
    level: 'info',
    message: payload.eventName,
    data: {
      storeSlug: sanitizeText(payload.storeSlug, 120),
      productSlug: sanitizeText(payload.productSlug, 120),
      surface: payload.metadata?.surface ?? payload.metadata?.source
    }
  })
}
...
export function trackBetaEvent(payload: BetaTelemetryPayload) {
  ...
  addTelemetryBreadcrumb(payload)
