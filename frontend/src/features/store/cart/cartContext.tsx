...
type Action =
  ...
  | { type: 'SET_ITEM_QUANTITY'; productId: string; qty: number }
  | { type: 'REMOVE_LINE_ITEM'; productId: string }
  | { type: 'CLEAR' }
  ...
  switch (action.type) {
    ...
    case 'SET_ITEM_QUANTITY': {
      ...
      const nextItems = state.items.map((item) =>
        item.productId === action.productId ? { ...item, qty: action.qty } : item
      )
      return {
        storeSlug: nextItems.length === 0 ? null : state.storeSlug,
        items: nextItems
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<CartContextValue>(() => ({
    ...
    setItemQuantity: (productId, qty) => dispatch({ type: 'SET_ITEM_QUANTITY', productId, qty }),
    removeLineItem: (productId) => dispatch({ type: 'REMOVE_LINE_ITEM', productId }),
