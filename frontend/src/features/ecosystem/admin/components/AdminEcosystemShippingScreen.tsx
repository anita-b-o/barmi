...
export default function AdminEcosystemShippingScreen() {
  ...
  return (
    <AdminLayout>
      <Breadcrumbs items={[{ label: 'Admin', href: routes.adminHome }, { label: 'Ecosystem', href: routes.adminEcosystem }, { label: 'Zonas de envío' }]} />
      <PageHeader
        title="Zonas de envío ECOSYSTEM"
        subtitle={me?.email}
        actions={(
          <>
      ...
      <ContextHeader
        ...
        description="La acción dominante es crear una zona. El listado queda para validar cobertura actual y remover zonas cuando sea necesario."
      />
      ...
      <Section title="Ecosystem activo">
        <Card>
          {ecosystemOptions.length === 0 && <EmptyState title="No hay ecosystems activos" description="Necesitás una membership activa para operar zonas de envío." />}
      ...
      <Section title="Zonas de envío">
        ...
        {!loading && sortedZones.length > 0 && (
          <DataTable
            headers={['Zona', 'Tipo', 'Cobertura', 'Costo', 'Moneda', 'Estado', 'Acciones']}
            rows={tableRows}
