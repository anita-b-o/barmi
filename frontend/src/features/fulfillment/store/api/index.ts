...
export const storeFulfillmentApi = {
  async list(auth: AuthRequestContext, options: { createdFrom?: string; createdTo?: string } = {}) {
    ...
    const data = await requestJsonWithAuth<unknown>(`/api/store/fulfillments${query ? `?${query}` : ''}`, {}, {}, auth)
