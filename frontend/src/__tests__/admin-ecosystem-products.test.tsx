...
describe('admin ecosystem products', () => {
  ...
  it('creates a product without sending unsupported fields', async () => {
    ...
    await setInputElementValue(nameInput, 'Té')
    await setInputElementValue(priceInput, '90')
  ...
  it('edits a product without sending unsupported fields', async () => {
    ...
    await setInputElementValue(nameInput, 'Alfajor premium')
    await setInputElementValue(priceInput, '140')

    const selects = Array.from(document.querySelectorAll('select'))
    const deliverySelect = selects[1]
    await setSelectElementValue(deliverySelect, 'false')
  ...
  it('deactivates a product with confirmation', async () => {
    const handler = mockFetch({
      '/api/auth/me': { body: authMe },
      '/api/ecosystem/admin/products?ecosystemId=eco-1&activeOnly=false': { body: listResponse },
    ...
    const deleteButton = Array.from(document.querySelectorAll('button')).find((button) => button.textContent === 'Desactivar')
    await clickElement(deleteButton)
    await flush()

    const confirmButton = Array.from(document.querySelectorAll('button'))
      .filter((button) => button.textContent === 'Desactivar')
      .at(-1)
